...
function mockAddress(i: number): string {
  return `0x${i.toString(16).padStart(4, "0")}${"a".repeat(36)}`;
}

function generateMockBankstrs(): Bankstr[] {
...
export interface IStorage {
  getCurrentAuction(): Promise<Bankstr>;
  getGallery(): Promise<Bankstr[]>;
  getStats(): Promise<Stats>;
}

export class MemStorage implements IStorage {
  private bankstrs: Bankstr[];

  constructor() {
    this.bankstrs = generateMockBankstrs();
  }

  async getCurrentAuction(): Promise<Bankstr> {
    return this.bankstrs[this.bankstrs.length - 1];
  }

  async getGallery(): Promise<Bankstr[]> {
    return this.bankstrs.filter((b) => b.settled);
  }
